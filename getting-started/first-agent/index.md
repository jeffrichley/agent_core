...
Config-driven path preferred in production
...
1. After each `start()`, the bus drains any persisted-but-pending envelopes addressed to that endpoint.
1. `sender.send(...)` calls `handle.publish(envelope)`. The handle stamps `from_="sender"`, runs `pre_publish` hooks, persists the envelope, then dispatches synchronously to `receiver.deliver(envelope)`.
1. `receiver.deliver()` appends to `.inbox` and calls `handle.ack(envelope.id)`.
1. `bus.stop()` calls `endpoint.stop()` in reverse registration order, then closes the store.
...
- [Running the daemon](https://jeffrichley.github.io/agent_core/getting-started/daemon/index.md) — run a supervised multi-endpoint process from `agent_core.yaml`
- [Concepts — Bus](https://jeffrichley.github.io/agent_core/concepts/bus/index.md) — envelope lifecycle, hooks, delivery guarantees
