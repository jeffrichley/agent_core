...
**Supervise endpoints.** The supervisor layer monitors endpoint health. If an endpoint raises during `start()` or repeatedly fails delivery, the supervisor backs off (exponential with jitter) and eventually quarantines the endpoint before probing it again. See [Endpoints](https://jeffrichley.github.io/agent_core/concepts/endpoints/index.md) for how this interacts with the deliver/ack contract.
