...
This step gates cutover. Both prerequisites have landed: MCP wiring (`0660b41` + `975c31d`) and the Pepper-facing briefs-author skill, which ships in the briefs package at `packages/agent-core-briefs/src/agent_core_briefs/skills/briefs-author/SKILL.md` (~1900 words, authored via `superpowers:writing-skills` TDD methodology — RED baseline + GREEN verify-with-different-scenario + REFACTOR for the CLI fetcher `env_passthrough` gotcha). The skill installs into `<agent_root>/.claude/skills/briefs-author/` at agent init time (project-scope, per-agent — never user-scope), keeping it bound to agents that actually use the briefs framework.
...
- **Briefs usage skill at `packages/agent-core-briefs/src/agent_core_briefs/skills/briefs-author/SKILL.md`** — *Done (~1900 words, authored via `superpowers:writing-skills`).* The skill ships inside the briefs package (mirrors the existing convention used by the `email` skill in `packages/core/src/agent_core/skills/email/`); agent init copies it into `<agent_root>/.claude/skills/briefs-author/` per-agent, project-scope, so only agents that actually use the briefs framework see it. Pepper-facing content documents how to author a playbook (YAML-in-MD format, simpleeval expression language, conditional sections, `${var}` substitution) and the gather config shape (fetchers list, namespace declarations, per-fetcher timeouts, `_errors` capture). Followed RED-GREEN-REFACTOR: baseline subagent run surfaced six framework gotchas (originally including "no auto-injected `now` namespace"; that one was subsequently closed by the `now` built-in fetcher landing — Invariant #1 in the skill now says "register the built-in `now` fetcher in your gather config"); GREEN-phase verification with a different scenario (`evening_recap` vs `lunch_check`) confirmed the skill closes the traps; REFACTOR added an explicit gotcha + common-mistakes entry for `CliFetcher` after the verify-phase agent invented an unsupported `env:` map.
