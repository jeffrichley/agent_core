...
class VoiceEndpoint:
    ...
    # NOTE: signatures MUST match agent_core.bus.protocol.Endpoint exactly —
    # deliver is single-arg (envelope only), start receives the BusHandle and
    # we stash it so deliver can ack stray envelopes. Mirrors WebcamEndpoint.
