...
5. **Unify auto-echo across TextMessage and ToolInvocation handlers in discord-pepper.** Spec lines 168 and 272 originally listed the TextMessage envelope handler alongside the ToolInvocation verbs as a call site to migrate. During Task 13 implementation (commit `ede2752`), the migration consolidated resolution into `_dispatch` (the ToolInvocation entry point) via a single pre-pydantic injection step. The TextMessage handler in `_deliver_text_message` was not migrated; it retains its pre-existing parallel implementation. Calibrated-criterion check (Wren/Pepper, 2026-05-12) found no named TextMessage-path symptom — Pepper's bug-today was ToolInvocation-shaped (she hardcoded `channel_id` in TextMessage envelopes, so the handler did exactly what she told it; the named demand is for A-side surfacing + C-side auto-echo on ToolInvocations). Three specific gaps to capture for the future-author:
   - **Different cache.** TextMessage handler uses `_inbound_envelope_discord` (discord-message-id → channel map); resolver uses `_recent_inbounds` (envelope-id → Envelope). Functionally analogous, structurally parallel.
   - **`outbound_channel_id` static fallback.** TextMessage handler has a third-tier fallback to `self.outbound_channel_id` that the spec's "refuse to guess" floor doesn't account for. **Explicit risk note:** removing this fallback is a behavior change. Before removal, either confirm zero operator config relies on it (search `agent_core.yaml` + endpoint config across known deployments) OR introduce a deprecation path (preserve under a `legacy_outbound_channel_id` flag, document removal in a major version).
   - **Divergent error message.** TextMessage hard-error message is `"TextMessage requires metadata.discord.channel_id or endpoint outbound_channel_id"`; the resolver's unified message is `"cannot determine channel — set metadata.discord.channel_id explicitly, or set in_reply_to so auto-echo can resolve."` Unify after #83 lands.
