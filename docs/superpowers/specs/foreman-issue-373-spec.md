...
**VACUUM INTO as the snapshot primitive.** SQLite's `VACUUM INTO '<path>'` produces a consistent, compacted copy of the live database — including all committed WAL frames — in a single atomic operation. It is the correct primitive for a live WAL-mode database: a naive `shutil.copy2` on a WAL DB can produce a torn copy if the WAL is being flushed concurrently. `aiosqlite` runs the `VACUUM INTO` call in a thread-pool executor so it does not block the asyncio event loop. No new dependency is needed: `aiosqlite>=0.20` is already declared in `packages/core/pyproject.toml`.

**Snapshot filename and retention.** Files are named `<store_stem>-YYYYMMDD-HHMMSS.sqlite` (UTC, e.g. `bus-20260717-140000.sqlite`). `apply_retention` parses timestamps from filenames with `datetime.strptime`, groups them by `(year, month, day, hour)` (for hourly) and `(year, month, day)` (for daily), takes the newest snapshot per group (groups are already newest-first via `list_snapshots` sort), then slices to 24 hourly groups and 14 daily groups. The union of those two kept sets is retained; every other snapshot is `path.unlink()`-ed. Files whose names don't match the pattern are left untouched.

**Scheduler DB discovery.** The default `scheduler.db` lives at `~/.agent-core/scheduler.db` — adjacent to `bus.sqlite` (confirmed via `_default_db_path()` in `packages/core/src/agent_core/endpoints/scheduler.py:183`). The backup loop checks `bus.config.storage_path.parent / "scheduler.db"` and includes it only when it exists. This is YAGNI — a future config key can add arbitrary extra paths if the scheduler DB ever moves.

**Config integration.** `backup_dir` is read from `bus.backup_dir` in the YAML (same flat block as `storage_path`). `None` (key absent) disables backup entirely, so deployments without an off-volume drive don't spin up a no-op task. The YAML key uses a string path that `runner.py` expands with `.expanduser()`.

**`restore` CLI safety.** Integrity check before swap ensures the operator isn't restoring a corrupt snapshot. `shutil.copy2` on a VACUUM-created snapshot is safe: the snapshot is a clean, WAL-free SQLite file (no `-wal` or `-shm` sidecar). The `--yes` flag bypasses confirmation for scripted use. The command help string documents that the daemon must be stopped before restoring — enforcing this in code is fragile and cross-platform, so it is not attempted.

**Backup dir on a different volume.** The spec does not hardcode `E:\agent-core-backups`. The YAML key is the operator's responsibility — the spec documents the intent (off-volume) in a comment added to the default `agent_core.yaml`.
...
4. **Add `_backup_loop` to `_run_bus` in `packages/core/src/agent_core/bus/cli.py`** — parallel asyncio task (alongside `_ttl_loop`/`_redelivery_loop`); active only when `bus.config.backup_dir is not None`; calls `run_backup` every `backup_interval_seconds` seconds; exceptions logged and swallowed.

5. **Add `restore` command to `packages/core/src/agent_core/bus/cli.py`** — `agent-core bus restore <snapshot> <target> [--yes/-y]`; integrity check → confirm → `shutil.copy2`.
...
- Verifying that the backup volume is actually a different physical volume than the store — operator deployment concern documented in comments, not enforced in code.
