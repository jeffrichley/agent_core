...
2. **Create `packages/agent-core-qa/src/agent_core_qa/fixtures.py`** — define the `source_daemon` session-scoped fixture (TCP-idempotency check, daemon init, daemon start, health-poll, yield URL, teardown via daemon stop). Annotate correctly for eventual `mypy --strict` coverage.
